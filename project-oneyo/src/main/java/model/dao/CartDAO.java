...
//				customMk.getCustomMealkitId() + " " + customMk.getPrice()); 
	...
	public Cart findCartItem(int customerId) {
		...
		try {
			String query = "SELECT cm.customMkId, m.mkid, m.mkname, m.filename, cm.price, cm.quantity, cm.calorie "
			...
			while (rs.next())
			{
				...
				cmList.add(new CustomMealkit(new Mealkit(mkId, mkName, filename), customMkId, customerId, price, quantity, calorie));
			}
			findIngList(cmList);
