						...
						<c:forEach var="order" items="${orderList}">
							<tbody>
								<tr>
									...
									<td>
										<!-- 주문일자 --> ${order.getOrderDate()}
									</td>
									<td>
										<!-- 주문상태 --> <c:set var="status" value="${order.getStatus()}" />
										<c:choose>
									...
									<td>
										<!-- 가격 --> ${order.getTotalPrice()}
									</td>
									<td>
										<button type="button" id="share"
											class="btn btn-primary btn-xs" style="background-color: #60B5BC; border-color: #60B5BC">공유하기</button>
									</td>
									<td><c:if test="${status != 3}">
											<form name="f${order.getOrderId()}" method="get"
												action="<c:url value="/order/delete" />">
	...
	<footer id="tempaltemo_footer">
		<div class="w-100 py-3 footer-color">
			<div class="container">
				<div class="row pt-2">
					<div class="col-12">
						<p class="text-left text-light" style="color: black !important;">데이터베이스 프로그래밍 - 김경현, 신효경, 이혜준,
							전수민</p>
					</div>
				</div>
			</div>
